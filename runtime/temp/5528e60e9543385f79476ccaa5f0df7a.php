<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:90:"/Applications/MAMP/htdocs/bbbbbb/public/../application/admin/view/comment/goods_lists.html";i:1582206891;s:73:"/Applications/MAMP/htdocs/bbbbbb/application/admin/view/public/title.html";i:1581519893;}*/ ?>
<!DOCTYPE html>
<html>
<head>
 <meta charset="utf-8">
          ...
          <div style="position: relative;">
            ...
            <form class="layui-form" method="get" style="display:inline-block;position: absolute;right: 0;">
              <div class="layui-input-inline">
                <select name="type">
                  <option value="1" <?php if($get['type'] == 1): ?>select<?php endif; ?>>用户名称</option>
                  <option value="2" <?php if($get['type'] == 2): ?>select<?php endif; ?>>商品名称</option>
                </select>
              </div>
              <div class="layui-input-inline">
      ...
      <table class="layui-table">
          ...
          <?php else: foreach($list as $l): ?>
              <tr>
                ...
                <td><?php if($l['is_praise'] == 1): ?>好评<?php elseif($l['is_praise'] == 2): ?>中评<?php else: ?>差评<?php endif; ?></td>
                <td><?php echo date('Y-m-d H:i:s',$l['create_t']); ?></td>
                <td><?php echo $l['device']; ?></td>
                <td>
  ...
  <script>
  	function del(id)
  	{
  		layer.confirm("您确定删除该评论吗？",function(){
  		  $.post("<?php echo url('comment/del'); ?>",{id:id},function (res) {
    ...
    function delAll()
    {
      ...
      layer.confirm("确认删除选择的评论吗？",function(){
        $.post("<?php echo url('comment/del'); ?>",{id:JSON.stringify(datas)},function (res) {
          layer.msg(res.msg,{icon:res.icon,time:1000},function(){
            window.location.reload();
          });
        },'json');
