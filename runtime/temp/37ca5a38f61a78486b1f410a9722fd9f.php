<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:81:"/Applications/MAMP/htdocs/bbbbbb/public/../application/admin/view/gain/lists.html";i:1582624258;s:73:"/Applications/MAMP/htdocs/bbbbbb/application/admin/view/public/title.html";i:1581519893;}*/ ?>
<!DOCTYPE html>
<html>
<head>
 <meta charset="utf-8">
    ...
    <div class="layui-tab layui-tab-brief">
      <crmblok>
          <div style="position: relative;">
            ...
            <button class="layui-btn layui-btn-normal" onclick="crm_admin_show('添加','<?php echo url('gain/add'); ?>')"><i class="layui-icon">&#xe608;</i>添加</button>
...
<!--                  <option value="2" <?php if($get['type'] == 2): ?>selected<?php endif; ?>>领域</option>-->
      ...
      <table class="layui-table">
          ...
          <?php if(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty())): ?>
            <tr align="center" style="height:500px;"><td colspan="8">暂无成果</td></tr>
          <?php else: foreach($list as $l): ?>
              <tr>
                ...
                <td><?php echo date('Y-m-d H:i:s',$l['get_time']); ?></td>
                <td><?php echo date('Y-m-d H:i:s',$l['create_t']); ?></td>
                <td>
                	<button class="layui-btn layui-btn-normal layui-btn-sm" onclick="crm_admin_show('编辑','<?php echo url('gain/edit',['id'=>$l['id']]); ?>')">编辑</button>
  ...
  <script>
  	function del(id)
  	{
  		layer.confirm("您确定删除该成果吗？",function(){
  		  $.post("<?php echo url('gain/del'); ?>",{id:id},function (res) {
    ...
    function delAll() {
      ...
      layer.confirm("确认删除选择的成果吗？",function(){
        $.post("<?php echo url('gain/del'); ?>",{id:JSON.stringify(datas)},function (res) {
          layer.msg(res.msg,{icon:res.icon,time:1000},function(){
            window.location.reload();
          });
        },'json');
