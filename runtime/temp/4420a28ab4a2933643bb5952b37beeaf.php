<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:83:"/Applications/MAMP/htdocs/bbbbbb/public/../application/admin/view/device/lists.html";i:1582638062;s:73:"/Applications/MAMP/htdocs/bbbbbb/application/admin/view/public/title.html";i:1581519893;}*/ ?>
<!DOCTYPE html>
<html>
<head>
 <meta charset="utf-8">
    ...
    <div class="layui-tab layui-tab-brief">
      <crmblok>
          <div style="position: relative;">
            ...
            <button class="layui-btn layui-btn-normal" onclick="crm_admin_show('添加','<?php echo url('device/add'); ?>')"><i class="layui-icon">&#xe608;</i>添加</button>
            <form class="layui-form" method="get" style="display:inline-block;position: absolute;right: 0;">
              <div class="layui-input-inline">
                <select name="type">
                  <option value="1" <?php if($get['type'] == 1): ?>selected<?php endif; ?>>设备名称</option>
                </select>
              </div>
              <div class="layui-input-inline">
      ...
      <table class="layui-table">
          ...
          <?php else: foreach($list as $l): ?>
              <tr>
                ...
                <td>
                	<button class="layui-btn layui-btn-normal layui-btn-sm" onclick="crm_admin_show('编辑','<?php echo url('device/edit',['id'=>$l['id']]); ?>')">编辑</button>
  ...
  <script>
  	function del(id)
  	{
  		layer.confirm("您确定删除该设备吗？",function(){
  		  $.post("<?php echo url('device/del'); ?>",{id:id},function (res) {
    ...
    function delAll() {
      ...
      layer.confirm("确认删除选择的设备吗？",function(){
        $.post("<?php echo url('device/del'); ?>",{id:JSON.stringify(datas)},function (res) {
          layer.msg(res.msg,{icon:res.icon,time:1000},function(){
            window.location.reload();
          });
        },'json');
