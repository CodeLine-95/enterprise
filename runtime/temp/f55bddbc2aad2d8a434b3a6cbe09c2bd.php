<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:87:"/Applications/MAMP/htdocs/bbbbbb/public/../application/admin/view/enterprise/lists.html";i:1582637413;s:73:"/Applications/MAMP/htdocs/bbbbbb/application/admin/view/public/title.html";i:1581519893;}*/ ?>
<!DOCTYPE html>
<html>
<head>
 <meta charset="utf-8">
    ...
    <div class="layui-tab layui-tab-brief">
      <crmblok>
          <div style="position: relative;">
            ...
            <button class="layui-btn layui-btn-normal" onclick="crm_admin_show('添加','<?php echo url('enterprise/add'); ?>')"><i class="layui-icon">&#xe608;</i>添加</button>
            <form class="layui-form" method="get" style="display:inline-block;position: absolute;right: 0;">
              <div class="layui-input-inline">
                <select name="type">
                  <option value="1" <?php if($get['type'] == 1): ?>selected<?php endif; ?>>企业名称</option>
                </select>
              </div>
              <div class="layui-input-inline">
      ...
      <table class="layui-table">
          ...
          <?php else: foreach($list as $l): ?>
              <tr>
                ...
                <td><?php echo $l['tel_name']; ?></td>
                <td><?php echo $l['tel']; ?></td>
                <td><?php echo $l['address']; ?></td>
                <td><?php echo $l['corporation']; ?></td>
                <td>
                  <?php switch($l['status']): case "1": ?>合作中<?php break; default: ?>已解约
                  <?php endswitch; ?>
                </td>
                <td>
                	<button class="layui-btn layui-btn-normal layui-btn-sm" onclick="crm_admin_show('编辑','<?php echo url('enterprise/edit',['id'=>$l['id']]); ?>')">编辑</button>
  ...
  <script>
  	function del(id)
  	{
  		layer.confirm("您确定删除该企业吗？",function(){
  		  $.post("<?php echo url('enterprise/del'); ?>",{id:id},function (res) {
    ...
    function delAll() {
      ...
      layer.confirm("确认删除选择的企业吗？",function(){
        $.post("<?php echo url('enterprise/del'); ?>",{id:JSON.stringify(datas)},function (res) {
          layer.msg(res.msg,{icon:res.icon,time:1000},function(){
            window.location.reload();
          });
        },'json');
